...
async function processAction(msg, cfg) {
  ...
  if (context.__eio_eval_result) {
    wrapped.logger.info('Message data did match filter condition');
    await wrapped.emit('data', msg);
  } else {
    wrapped.logger.info('Message data did not match filter condition');
    if (rejectRoutingKey) {
      const rejectFlowID = cfg.reject;
      const hookURL = (process.env.HOOKS_URL || 'https://in.elastic.io/hook/') + rejectFlowID;
      wrapped.logger.info('Your message will be rejected to flowID=%s');
      ...
      try {
        ...
        wrapped.logger.info('Message sent to reject processing flow');
      } catch (err) {
